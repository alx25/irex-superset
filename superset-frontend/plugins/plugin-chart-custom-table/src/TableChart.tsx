...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  // Debug logging para verificar los props
  console.log('🔥 TableChart Props Debug:', {
  ...
  const getColumnConfigs = useCallback(
    (
      ...
      console.log('🔍 Dynamic column names check:', {
        enableDynamicColumnNames,
        columnNameTemplate,
        columnKey: key
      });
      ...
      if (enableDynamicColumnNames && columnNameTemplate) {
        console.log('🚀 Calling generateDynamicColumnName for column:', key);
        try {
          const dynamicName = generateDynamicColumnName(
            columnNameTemplate,
            {
              ...jinjaFieldValues,
              column_name: key,          // The actual column key like 'SUM(sell_in)'
              original_name: originalLabel,  // Original label
              transformed_name: label,   // Current label (possibly custom)
            },
            filterValues, // Pass filter values for {% set %} statements
          );
