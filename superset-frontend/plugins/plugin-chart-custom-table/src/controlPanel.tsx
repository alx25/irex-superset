  ...
  controlPanelSections: [
        ...
        [
          {
            ...
            config: {
              ...
              description: t('Additional metrics to use in column name templates (these won\'t appear as columns in the table)'),
              multi: true,
              visibility: isAggMode,
              resetOnHide: false,
              mapStateToProps: (
                ...
                columns: datasource?.columns[0]?.hasOwnProperty('filterable')
                  ...
                  : datasource?.columns,
                savedMetrics: defineSavedMetrics(datasource),
                selectedMetrics: form_data.jinja_fields || [],
                datasource,
              }),
            },
          },
    ...
    },
    {
      ...
      controlSetRows: [
        ...
        [
          {
            ...
            config: {
              ...
              default: '',
              description: t(
                'Jinja template for column names. Available variables: {{data}}, {{metrics}}, {{column_name}}, {{first_row}}, {{last_row}}, {{row_count}}. Example: "{{column_name}} ({{row_count}} rows)". NOTE: If this field appears empty after page reload, your template is still saved and working correctly. Check the browser console for "Template applied:" logs to confirm.',
              ),
        ...
        [
          {
            name: 'show_template_helper',
            config: {
              type: 'SelectControl',
              label: t('Template Helper'),
              renderTrigger: false,
              clearable: true,
              searchable: false,
              default: null,
              ...
              description: t('Tu template funciona perfectamente. Usa estas opciones para ayuda y acceso al código.'),
              visibility: ({ controls }) => Boolean(controls?.enable_dynamic_column_names?.value),
              validators: [],
              // Simplified change handler focused on guidance
              onChange: (value: string, formData: any, onChange: any) => {
                if (value) {
                  setTimeout(() => {
                    if (value === 'console') {
                      alert(`🔍 CÓMO VER TU TEMPLATE EN LA CONSOLA:\n\n═══════════════════════════════════\n\n1️⃣ Presiona F12 para abrir DevTools\n\n2️⃣ Haz clic en la pestaña "Console"\n\n3️⃣ Busca la línea que dice:\n   "Original template:"\n\n4️⃣ Ahí verás tu código completo:\n   {% if column_name == 'SUM(sell_out)' %}\n   Ventas del año {{MAX(anio_id)}}\n   ...\n\n5️⃣ Selecciona todo el código y cópialo\n\n═══════════════════════════════════\n\n✅ TU TEMPLATE FUNCIONA PERFECTAMENTE\nEsto es solo para acceder al código cuando lo necesites.`);
                    } else if (value === 'examples') {
                      alert(`💡 EJEMPLOS DE TEMPLATES JINJA:\n\n═══════════════════════════════════\n\n🔥 TU TEMPLATE ACTUAL (funciona perfecto):\n{% if column_name == 'SUM(sell_out)' %}\nVentas del año {{MAX(anio_id)}}\n{% elif column_name == 'jefe_marca' %}\nJefe de Marca\n{% else %}\n{{transformed_name}}\n{% endif %}\n\n═══════════════════════════════════\n\n📝 OTROS EJEMPLOS ÚTILES:\n\n• Básico con contador:\n{{column_name}} ({{row_count}} filas)\n\n• Con año dinámico:\nVentas {{MAX(anio_id)}}\n\n• Condicional por valor:\n{% if MAX(anio_id) >= 2024 %}\n{{column_name}} - Actual\n{% else %}\n{{column_name}} - Histórico\n{% endif %}\n\n• Por tipo de columna:\n{% if 'ventas' in column_name %}\n💰 {{column_name}}\n{% elif 'jefe' in column_name %}\n👤 {{column_name}}\n{% else %}\n📊 {{column_name}}\n{% endif %}`);
                    } else if (value === 'help') {
                      alert(`❓ AYUDA COMPLETA - SISTEMA JINJA FIELDS\n\n═══════════════════════════════════\n\n✅ TU SISTEMA ESTÁ FUNCIONANDO PERFECTAMENTE\nLos nombres de columnas se actualizan en tiempo real.\n\n═══════════════════════════════════\n\n📋 PARA COPIAR TU TEMPLATE:\n1. F12 → Console → Buscar "Original template:"\n2. Copiar código de la consola\n3. Pegarlo donde necesites\n\n═══════════════════════════════════\n\n� VARIABLES DISPONIBLES:\n• {{column_name}} - Nombre original\n• {{row_count}} - Total de filas\n• {{MAX(metrica)}} - Máximo de métrica\n• {{SUM(metrica)}} - Suma de métrica\n• {{MIN(metrica)}} - Mínimo de métrica\n• {{AVG(metrica)}} - Promedio de métrica\n• {{data}} - Datos completos\n• {{first_row}} - Primera fila\n• {{last_row}} - Última fila\n\n═══════════════════════════════════\n\n⚡ SINTAXIS CONDICIONAL:\n{% if condicion %}\n  resultado si verdadero\n{% elif otra_condicion %}\n  resultado si la segunda es verdadera\n{% else %}\n  resultado por defecto\n{% endif %}\n\n═══════════════════════════════════\n\n🎯 CONSEJOS:\n• Agrega métricas en "Jinja Fields"\n• Guarda el dashboard después de cambios\n• Revisa la consola para debug\n• El sistema funciona aunque el campo parezca vacío`);
                    }
        ...
        [
          {
            ...
            config: {
              ...
              default: false,
              resetOnHide: false,
              mapStateToProps: ({ controls, form_data }) => ({
                value: form_data?.use_metric_in_column_names || false,
                disabled: !Boolean(controls?.enable_dynamic_column_names?.value),
              }),
              description: t(
