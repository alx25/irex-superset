...
const processColumns = memoizeOne(function processColumns(
  ...
  // Process jinja_fields to extract their values from the dataset
  const jinjaFields = (props.rawFormData.jinja_fields ?? []).map(getMetricLabel);
  const jinjaFieldValues: Record<string, any> = {};
  
  console.log('Processing Jinja Fields:', jinjaFields);
  console.log('Raw jinja_fields from form data:', props.rawFormData.jinja_fields);
  console.log('All available record keys:', records && records.length > 0 ? Object.keys(records[0]) : 'No records');
  console.log('First few records:', records?.slice(0, 3));
  
  jinjaFields.forEach(fieldKey => {
    console.log(`Looking for jinja field "${fieldKey}" in data...`);
    if (records && records.length > 0) {
      const fieldValues = records.map(record => record[fieldKey]).filter(value => value != null);
      console.log(`Jinja field "${fieldKey}" has values:`, fieldValues);
      if (fieldValues.length > 0) {
        // For aggregated metrics, usually there's only one value per jinja field
        if (fieldValues.length === 1) {
          jinjaFieldValues[fieldKey] = fieldValues[0];
        } else {
          // If multiple values, try to get the max for numeric values
          const numericValues = fieldValues.filter(v => typeof v === 'number') as number[];
          if (numericValues.length > 0) {
            jinjaFieldValues[fieldKey] = Math.max(...numericValues);
          } else {
            // Fall back to first value for non-numeric
            jinjaFieldValues[fieldKey] = fieldValues[0];
          }
        }
      }
  ...
  console.log('Final jinjaFieldValues:', jinjaFieldValues);
  ...
  const columns: DataColumnMeta[] = (colnames || [])
    .filter(
      key =>
        ...
        // Filter out jinja_fields - they shouldn't appear as columns
        !jinjaFields.includes(key),
    )
    .map((key: string, i) => {
  ...
  return [metrics, percentMetrics, columns, jinjaFieldValues] as [
    typeof metrics,
    typeof percentMetrics,
    typeof columns,
    typeof jinjaFieldValues,
  ];
}, isEqualColumns);
...
const transformProps = (
  ...
  const [metrics, percentMetrics, columns, jinjaFieldValues] = processColumns(chartProps);
